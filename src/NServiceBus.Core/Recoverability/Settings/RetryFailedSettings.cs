namespace NServiceBus
{
    ...
    public class RetryFailedSettings : ExposeSettings
    {
        ...
        public RetryFailedSettings OnMessageSentToErrorQueue(Func<FailedMessage, CancellationToken, Task> notificationCallback)
        {
            ...
            subscriptions.MessageFaultedNotification.Subscribe((faulted, token) =>
            {
                var headerCopy = new Dictionary<string, string>(faulted.Message.Headers);
                var bodyCopy = faulted.Message.Body.Copy();
                return notificationCallback(new FailedMessage(faulted.Message.MessageId, headerCopy, bodyCopy, faulted.Exception, faulted.ErrorQueue), token);
            });
