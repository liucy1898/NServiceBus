namespace NServiceBus
{
    ...
    public class ImmediateRetriesSettings : ExposeSettings
    {
        ...
        public ImmediateRetriesSettings OnMessageBeingRetried(Func<ImmediateRetryMessage, CancellationToken, Task> notificationCallback)
        {
            ...
            subscriptions.MessageRetryNotification.Subscribe((retry, token) =>
            {
                ...
                return notificationCallback(new ImmediateRetryMessage(retry.Message.MessageId, headerCopy, bodyCopy, retry.Exception, retry.Attempt), token);
            });
